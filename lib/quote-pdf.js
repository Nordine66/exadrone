...
function buildQuotePdf({ quote, service, contact }) {
  return new Promise((resolve, reject) => {
    ...
    const colSurfaceX = marginX + contentW - 300
    const colUnitX = marginX + contentW - 210
    const colTotalX = marginX + contentW - 110

    doc.rect(marginX, tableY, contentW, 24).fill(BLUE)
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#FFFFFF')
      .text('PRESTATION', marginX + 12, tableY + 8)
      .text('SURFACE', colSurfaceX, tableY + 8, { width: 80, align: 'right' })
      .text('PRIX UNIT. HT', colUnitX, tableY + 8, { width: 80, align: 'right' })
      .text('TOTAL HT', colTotalX, tableY + 8, { width: marginX + contentW - colTotalX - 12, align: 'right' })

    const rowH = 46
    const rowY = tableY + 24
    doc.rect(marginX, rowY, contentW, rowH).fillAndStroke(PANEL, BORDER)
    doc.font('Helvetica-Bold').fontSize(10).fillColor(DARK)
      .text(service.label, marginX + 12, rowY + 10, { width: colSurfaceX - marginX - 20 })
    if (service.detail) {
      doc.font('Helvetica').fontSize(8).fillColor(GRAY)
        .text(service.detail, marginX + 12, rowY + 26, { width: colSurfaceX - marginX - 20 })
    }
    const rawSubtotal = round2(quote.surface * quote.unitPriceHT)
    doc.font('Helvetica').fontSize(9.5).fillColor(DARK)
      .text(`${quote.surface} m²`, colSurfaceX, rowY + 16, { width: 80, align: 'right' })
      .text(`${fmtMoney(quote.unitPriceHT)}/m²`, colUnitX, rowY + 16, { width: 80, align: 'right' })
      .font('Helvetica-Bold')
      .text(fmtMoney(rawSubtotal), colTotalX, rowY + 16, { width: marginX + contentW - colTotalX - 12, align: 'right' })
