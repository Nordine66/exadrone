...
**FFmpeg and FFprobe must be available on PATH. Check with `ffmpeg -version` / `ffprobe -version` before starting; install via Homebrew (`brew install ffmpeg`) if missing.**
